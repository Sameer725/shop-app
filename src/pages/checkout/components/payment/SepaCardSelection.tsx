...
export const SepaCardSelection: React.FC = () => {
  ...
  const navigateToAddCard = () => navigation.navigate(PaymentScreens.SEPA_PAY_NEW_SCREEN);

  useEffect(() => {
    const cards = route.params.data.filter((card) => card.type === CardType.SEPA) ?? [];
